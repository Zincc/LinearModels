...
alpha <- 0.7
k <- 3 # licznoœæ pierwszej cechy
m <- 4 # licznoœæ drugiej cechy
g <- 900 # liczba obserwacji w ka¿dej grupie
N <- k*m*g # ³¹czna liczba obserwacji
ILE <- 2000 # liczba powtórzeñ symulacji F-testu
istotnosc <- 0.05
```
...
```
£atwo sprawdziæ, ¿e ró¿nica œrednich miêdzy klasami 1 i 2 oraz 1 i 3 w grupie A wynosi ok. 0.7.
ANOVA potwierdza nasz¹ intuicjê, ¿e œrednie w grupach A i B s¹ istotnie ró¿ne.
```{r}
...
```
Moc testu na poziomie istotnoœci 0.05 wynosi
```{r}
mean(FTest_1>istotnosc)
```

##Czêœæ2
Ta czêœæ jest bardzo podobna do poprzedniej. W zasadzie liczymy dok³adnie to samo dla trochê zmodyfikowanego zbioru.
...
```
##Czêœæ3
Zgodnie z tabelk¹ na stronie 89, tworzê przyk³ad, w którym alfa1 = 0.7 oraz  gamma1=0.7.
```{r,message=FALSE,warning=FALSE}
set.seed(8)
...
```
SprawdŸmy, czy faktycznie nasz zbiór spe³nia warunki alfa1 = 0.7 oraz gamma1=0.7
```{r}
