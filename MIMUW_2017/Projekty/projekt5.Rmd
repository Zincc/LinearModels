...
```{r}
attach(actionTimeScoreGender)
countries=levels(CNT)
new<- filter(actionTimeScoreGender, CNT %in% countries[1:20])
tst<- new %>% group_by(CNT, ST004D01T) %>% summarise(time=mean(sqrt(T)))
p=ggplot(new, aes(x = factor(CNT), y = sqrt(T), colour = ST004D01T)) + 
...
new<- filter(actionTimeScoreGender, CNT %in% countries[21:40])
tst<- new %>% group_by(CNT, ST004D01T) %>% summarise(time=mean(sqrt(T)))
p=ggplot(new, aes(x = factor(CNT), y = sqrt(T), colour = ST004D01T)) + 
    geom_point(data = tst, aes(y = time)) +
    geom_line(data = tst, aes(y = time, group = ST004D01T)) + 
...
new<- filter(actionTimeScoreGender, CNT %in% countries[41:59])
tst<- new %>% group_by(CNT, ST004D01T) %>% summarise(time=mean(sqrt(T)))
p=ggplot(new, aes(x = factor(CNT), y = sqrt(T), colour = ST004D01T)) + 
    geom_point(data = tst, aes(y = time)) +
    geom_line(data = tst, aes(y = time, group = ST004D01T)) + 
...
```{r}
book=levels(BOOKID)
new<- filter(actionTimeScoreGender, BOOKID %in% book[1:22])
tst<- new %>% group_by(BOOKID, ST004D01T) %>% summarise(time=mean(sqrt(T)))
p=ggplot(new, aes(x = factor(BOOKID), y = sqrt(T), colour = ST004D01T)) + 
    geom_point(data = tst, aes(y = time)) +
    geom_line(data = tst, aes(y = time, group = ST004D01T)) + 
    theme_bw()+theme(axis.text.x = element_text(angle = 90, hjust = 1))+
...
book=levels(BOOKID)
new<- filter(actionTimeScoreGender, BOOKID %in% book[23:44])
tst<- new %>% group_by(BOOKID, ST004D01T) %>% summarise(time=mean(sqrt(T)))
p=ggplot(new, aes(x = factor(BOOKID), y = sqrt(T), colour = ST004D01T)) + 
    geom_point(data = tst, aes(y = time)) +
    geom_line(data = tst, aes(y = time, group = ST004D01T)) + 
